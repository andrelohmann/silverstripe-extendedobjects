...
/**
 * extended_image/code/ExtendedImage.php
 * 
...
class ExtendedImage extends DataExtension {
	...
	public function getMergedImage($format, $padding, $mergeimage) {
            if($this->owner->ID && $this->owner->Filename && Director::fileExists($this->owner->Filename) && Director::fileExists($mergeimage)) {
	...
	public function generateMergedImage($padding, $bgImagePath, $overlayImagePath, $cacheFile){
            ...
            $frontImage = $this->owner->generateSetRatioSize($ovImage, ($bgImage->getWidth() - (2*$padding)), ($bgImage->getHeight() - (2*$padding)));
         ...
         */
        public function ColoredSetSize($width, $height, $background = "FFFFFF") {
                return (($this->owner->getWidth() == $width) &&  ($this->owner->getHeight() == $height)) 
			? $this->owner
			: $this->owner->getColoredFormattedImage('ColoredSetSize', $width, $height, $background);
	}
	/**
         * On previous Versions Parameters were limited to the number of 2
         * public function ColoredSetSize($widthheight, $background = "FFFFFF") {
                $wh = explode(':', $widthheight);
		return $this->owner->getColoredFormattedImage('ColoredSetSize', $wh[0], $wh[1], $background);
	}*/
	 ...
	 * @return Image_Cached
	 */
	public function getColoredFormattedImage($format, $arg1 = null, $arg2 = null, $background = "FFFFFF") {
		if($this->owner->ID && $this->owner->Filename && Director::fileExists($this->owner->Filename)) {
			$cacheFile = $this->owner->cacheColoredFilename($format, $arg1, $arg2, $background);

			if(!file_exists(Director::baseFolder()."/".$cacheFile) || isset($_GET['flush'])) {
				$this->owner->generateColoredFormattedImage($format, $arg1, $arg2, $background);
			}
	 ...
	 * @return string
	 */
	public function cacheColoredFilename($format, $arg1 = null, $arg2 = null, $background = "FFFFFF") {
		$folder = $this->owner->ParentID ? $this->owner->Parent()->Filename : ASSETS_DIR . "/";
		
		$format = $format.$arg1.$arg2.$background;
	 ...
	 * @param string $background Background-Color of the resized Image
	 */
	public function generateColoredFormattedImage($format, $arg1 = null, $arg2 = null, $background = "FFFFFF") {
		$cacheFile = $this->owner->cacheColoredFilename($format, $arg1, $arg2, $background);
		...
		if($backend->hasImageResource()){
			...
			if($this->owner->hasMethod($generateFunc)){
				$backend = $this->owner->$generateFunc($backend, $arg1, $arg2, $background);
				if($backend){
					$backend->writeTo(Director::baseFolder()."/" . $cacheFile);
				}
	
			} else {
				USER_ERROR("Image::generateColoredFormattedImage - Image $format function not found.",E_USER_WARNING);
			}
		}
	}
	 ...
	 * @return GD
	 */
	public function generateColoredSetSize(Image_Backend $backend, $width, $height, $background = "FFFFFF") {
		if(!$backend){
			user_error("Image::generateColoredFormattedImage - generateColoredSetSize is being called by legacy code"
